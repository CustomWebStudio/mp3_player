...
var apiURL='https://api.vk.com/method/audio.get?access_token=0c219dfae1cd749bf330c03e0c8799b09efb7b5f6fe0a406c87b11fc52129d529de0b2ec8159414ef2be0&callback=?';
$.getJSON(apiURL,function(data){
	var i=0;
	var vk_playlist=[];
	$.each( data.response, function(i, audio) {
		vk_playlist.push({'file':audio.url});
		});
	opt.vk_jplaylist=JSON.stringify(vk_playlist);
        ...
        var make = function() {
            ...
            $playlist.hide();
            $plControl = $('<div class="pl_btn">'+locale.playlist+'</div>');
            ...
            opt.playlist.forEach(function(track, i) {
                if (!track.title || !track.artist) { //доповнюємо інфу метаданими з файлу
                    ID3v2.parseURL(track.file, function(tags) { //підгрузка метаданих з файлу
                        if (!track.title) {
                            track.title = tags.Title ? tags.Title : '&nbsp;';
                        } //назва
                        if (!track.artist) {
                            track.artist = tags.Artist ? tags.Artist : '&nbsp;';
                        } //виконавець
            ...
            $audio.loadTrack = function(n) { //ініціалізація конкретного трека
                ...
                if (n < 0) {
                    n = opt.playlist.length - 1;
                } //перший трек
                else if (n > opt.playlist.length - 1) {
                    n = 0;
                } //останній трек
                ...
                if (!opt.playlist[n].duration) {
                    opt.playlist[n].duration = 120;
                } //fix на неможливість визначити довжину треку
                ...
                if (!opt.playlist[n].cover || !opt.playlist[n].title || !opt.playlist[n].artist) { //доповнюємо інфу метаданими з файлу
                    ID3v2.parseURL(opt.playlist[n].file, function(tags) { //підгрузка метаданих з файлу
                        if (!opt.playlist[n].cover) {
                            opt.playlist[n].cover = tags.pictures.length ? tags.pictures[0].dataURL : opt.defCover;
                        } //обложка
                        if (!opt.playlist[n].title) {
                            opt.playlist[n].title = tags.Title ? tags.Title : '&nbsp;';
                        } //назва
                        if (!opt.playlist[n].artist) {
                            opt.playlist[n].artist = tags.Artist ? tags.Artist : '&nbsp;';
                        } //виконавець
                        $audio.loadTrack(n);
                    });
                } else {
                    ...
                    if (opt.playlist[n].duration) {
                        $progress.slider({
                            max: opt.playlist[n].duration
                        });
                    } //діапазон прогресбара	
                }
            }
            ...
            $audio.addEventListener('loadedmetadata', function(e) { //можна грати
                ...
                $album.animate({
                    width: '100%'
                }, 700); //розгортаємо альбом

                if (!$play.hasClass('pause')) {
                    $audio.play();
                } //запускаємо, якщо не пауза
            });
            ...
            $audio.addEventListener('timeupdate', function() {
                $record.rotate({
                    animateTo: $this.secToDeg($audio.currentTime)
                }); //крутипо платівку
            ...
            $next.click(function() {
                ...
                if ($random.hasClass('active')) {
                    while (num == $audio.currentTrack) {
                        num = Math.floor(Math.random() * (opt.playlist.length));
                    }
                } else {
